...
export function createSupabaseAuthAdapter(auth) {
    ...
    const normalize = (data) => ({ ...data, user: publicUser(data?.user), session: data?.session ? { ...data.session, user: publicUser(data.session.user) } : null });
    return {
        ...
        onAuthStateChange: callback => auth.onAuthStateChange((event, session) => callback({ event, session: session ? { ...session, user: publicUser(session.user) } : null })).data.subscription,
    };
}
