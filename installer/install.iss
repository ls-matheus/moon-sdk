...
Filename: "{sys}\WindowsPowerShell\v1.0\powershell.exe"; Parameters: "-NoLogo -NoProfile -ExecutionPolicy Bypass -Command ""$ErrorActionPreference='Stop'; $npm=(Get-Command npm.cmd -ErrorAction Stop).Source; $target='{app}\sdk\bin\moon.mjs'; if (!(Test-Path $target)) {{ throw 'O arquivo principal do Moon SDK não foi encontrado.' }}; & $npm install --global '{app}\sdk' '@base44/sdk' --force; $bin=Join-Path $env:APPDATA 'npm'; $q=[char]34; New-Item -ItemType Directory -Path $bin -Force | Out-Null; Set-Content -Path (Join-Path $bin 'moon.cmd') -Encoding ascii -Value ('@echo off' + [Environment]::NewLine + 'node ' + $q + $target + $q + ' %*'); Set-Content -Path (Join-Path $bin 'moon.ps1') -Encoding ascii -Value ('& node ' + $q + $target + $q + ' @args'); if (!(Test-Path (Join-Path $bin 'moon.cmd'))) {{ throw 'O launcher Moon não foi criado.' }}"""; StatusMsg: "Instalando Moon SDK, Base44 SDK e dependências..."; Flags: runhidden waituntilterminated
