...
export interface AuthAdapter {
  ...
  signInWithOAuth?(options: { provider: string; options?: Record<string, unknown> }): Promise<void>;
  verifyOtp?(params: Record<string, unknown>): Promise<void>;
  resend?(params: Record<string, unknown>): Promise<void>;
  onAuthStateChange(callback: (change: AuthChange) => void): { unsubscribe(): void };
}
...
export interface MoonClient {
  auth: {
    ...
    hasToken(): boolean;
    onChange(callback: (change: AuthChange) => void): { unsubscribe(): void };
